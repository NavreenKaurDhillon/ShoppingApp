...
import com.example.demopaginationapp.databinding.FragmentBrandBinding
...
class BrandFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        ...
        binding.rvBrands.layoutManager = GridLayoutManager(context, 3)
        ...
        viewModel.products.observe(viewLifecycleOwner) { resource ->
            when (resource.status) {
                ...
                Status.SUCCESS -> {
                    ...
                    resource.data?.products?.let { adapter.submitList(it) }
                }
                Status.ERROR -> {
                    binding.progressBar.visibility = View.GONE
                }
